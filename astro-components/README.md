...
Props: `variant` (`suite` shows the full-brand logo headline; `default` names the current product for cross-sell, default `default`), `productName` (required when `variant="default"`), `products` (`{ prefix, suffix, desc, accent?, href? }[]`, required), `headingLevel` (`2` | `3` | `4`, default `2`), `label` (landmark name, default `Iron Suite`), `donateImgSrc`, `suiteLogoImgSrc`, `productLogoSrc`, `rainbowSrc` (asset paths, defaulting to site-root `/assets/…` — pass your own if they live elsewhere), `class`.

**The four asset paths default to the site root, and the rainbow is a prop for a reason.** A component cannot know where your images are, so it guesses `/assets/…` — right for an app served from its own origin, and overridable everywhere else. The previous defaults were *page-relative* (`assets/logo.svg`), which resolves against the current URL: correct on `/`, a silent 404 on `/pricing`. The backdrop was worse than a bad default — it was a bracketed `bg-` arbitrary value holding the url, so the path was compiled into the stylesheet where a consumer's bundler rewrote it to `/_astro/assets/Rainbow.svg`, it 404'd, the band rendered flat, and no prop existed to correct it (the ROI calculator carried an `!important` global override instead). The class is not spelled out here on purpose: Tailwind extracts candidates from prose too, and `npm run check:prose-classes` fails if it reappears anywhere this package ships. It is painted inline from `rainbowSrc` now. The docs are the one caller that overrides all four, because they publish under a repository subpath where a root path leaves the site.
